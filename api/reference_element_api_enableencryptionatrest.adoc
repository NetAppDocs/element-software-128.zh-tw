...
[NOTE]
====
* 若要查看叢集上靜態加密和/或靜態軟體加密的目前狀態，請使用下列方法：link:../api/reference_element_api_getclusterinfo["取得集群資訊方法"^] 。你可以使用 `GetSoftwareEncryptionAtRestInfo` link:../api/reference_element_api_getsoftwareencryptionatrestinfo["獲取集群用於加密靜態資料的資訊的方法"^]。
* 此方法無法實作靜態軟體加密。這只能透過使用以下方式完成：link:../api/reference_element_api_createcluster.html["建立集群方法"^]和 `enableSoftwareEncryptionAtRest`設定為 `true`。
